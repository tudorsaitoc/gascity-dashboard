# PRD: Bead-ID linked view — unify beads, PRs, issues, formula runs, sessions

## Problem Statement

The dashboard surfaces five entity types — beads, GitHub PRs, GitHub issues, formula runs, and agent sessions — but the operator cannot navigate between an entity and everything associated with it. The associations already exist in data: bead metadata carries `gc.root_bead_id`, `gc.parent_bead_id`, `molecule_id`, `pr_review.pr_number`, `bugflow.github_issue_number`, and `gc.scope_kind/scope_ref`; the supervisor's `/beads/graph/{rootID}` already computes a multi-source bead closure; and the codebase has independently built the same "inline cross-link → adjacent read-only surface" pattern three times (AgentDetail's assigned beads, Triage's `· slung →` link, FormulaRunDetail's per-node session links). What is missing is a single, reusable join layer and a consistent way to present it.
...
- Given any one entity (bead, PR, issue, formula run, session), surface the directly-adjacent associated entities, each navigable to re-center on it (one-hop drill, not a recursive neighborhood).
...
- **Build the join layer whole but small in v1.** Decisive argument: the minimal-inline approach does not avoid the hard problems (URL sanitisation at the bead trust boundary, unresolved/ambiguous rendering, cross-clock staleness, the `Fixes #N` ZFC quarantine) — it _distributes_ them across 3–5 inline copies with no compiler forcing agreement, and the 4th copy is where one silently forgets the allow-list. The codebase is already at the rule-of-three (the pattern exists 3× unnamed), and `shared/` wire-shape centralization is a stated project invariant. The index is a _promotion_ of the extraction already at `snapshot/collectors/runs.ts:244-321`, not a new subsystem.
- **Scope the index to bead-native / authoritative directions in v1** (parent/child, molecule membership, bead↔PR#, bead↔issue#, bead↔session, bead↔formula-run) — all from metadata the snapshot already extracts.
- **Render the component on the two surfaces that already have the data** (AgentDetail, FormulaRunDetail) in v1.
...
- **R3 — `GET /api/links/:ref` endpoint.** Resolves any input ref (`bead-id`, `pr/<n>`, `issue/<n>`, `<session-id>`, `<run-id>`) to its bead-id(s), runs the bead closure once, decorates with session/GitHub/run nodes, and returns an `EntityLinkView`. Sets `partial: true` if any contributing fetch fails (mirrors `routes/runs.ts:79-81`). Node payloads carry only display-summary fields (title, status, type, url), never full bodies.
...
- **R4 — Every rendered cross-entity URL passes an `^https?://` allow-list.** Bead metadata is a trust boundary (cf. `externalUrl` sanitisation, `runs.ts` gascity-dashboard-4x3); React does not strip `javascript:` from hrefs.
...
- **R9 — Jump-to-entity command.** One keyboard-first entry point (e.g. `/` then ref) that resolves a bead-id / session-alias / `pr/<n>` / run-id to its detail surface, giving the two "homeless" entities (GitHub PR/issue, formula/order run) a navigable destination.
...
1. Are bead IDs globally unique across the city, or only within a molecule/run? (Determines whether the index key needs `scope:ref` namespacing.)
...
Four independent lenses (prior-art, first-principles technical, UX/run, failure-modes/contrarian).
