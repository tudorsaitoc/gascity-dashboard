...
import { useWorkflowNodeSelection } from './useWorkflowNodeSelection';

afterEach(() => {
  cleanup();
});

describe('useWorkflowNodeSelection', () => {
  ...
  it('toggles one selected node and clears it on Escape', () => {
    const { result } = renderHook(() =>
      useWorkflowNodeSelection(detailWithNodes(['design', 'review']), null, routeKey(null)),
    );
...
function renderSelectionHook(initialProps: HookProps) {
  return renderHook(
    ({ detail, routeNodeId, routeKey }: HookProps) =>
      useWorkflowNodeSelection(detail, routeNodeId, routeKey),
    { initialProps },
  );
}
...
function detailWithNodes(nodeIds: string[]): WorkflowRunDetail {
  return {
    workflowId: 'wf-1',
    rootBeadId: 'wf-1',
...
function displayNode(id: string): WorkflowDisplayNode {
  return {
    id,
    semanticNodeId: id,
    title: titleFor(id),
