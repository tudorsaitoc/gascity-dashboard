...
## C1. SSE singleton (`backend/src/maintainer/sse.ts:15`)

- **What.** `const clients = new Set<Response>();` at module scope. `addSseClient`, `removeSseClient`, `notifyRefresh`, and `sendHeartbeat` are the only readers/writers, all closing over that Set. `backend/src/maintainer/worker.ts:6` imports `notifyRefresh` and `sendHeartbeat`; `backend/src/routes/maintainer.ts:27` imports `addSseClient`, `notifyRefresh`, `removeSseClient`.
