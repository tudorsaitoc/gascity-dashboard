...
> Note: the PRD §7 bullet ("`routes/maintainer.ts` imports `raceWithTimeout` from `routes/sessions.js`") is **slightly off** against the current tree — the import is actually in `app.ts`, used by app.ts to construct the `listSessions` injection passed into `maintainerRouter`. The coupling is real, but its surface is one level removed. Entry C3 below corrects the location.
...
- **Status (PR-B1):** RESOLVED. `sse.ts` moved into `backend/src/views/modules/maintainer/sse.ts`; the `// module-allow: SSE client registry is intentionally module-scoped per maintainer-coupling.md C1` line-comment marker is in place above the `Set` declaration. Both consumers (router, worker) import via the canonical sibling path `./sse.js` — no stale `../maintainer/sse.js` import remains in the tree.
...
## C3. `raceWithTimeout` cross-route import for the maintainer `listSessions` injection (`app.ts:15, 122-125`)

- **What.** `app.ts` imports `raceWithTimeout` from `./routes/sessions.js` (line 15) and uses it at lines 122-125 to wrap `gc.listSessions()` with a 3s timeout before injecting the result into `maintainerRouter` as the `listSessions` option. The helper itself is exported from `routes/sessions.ts:67`. The PRD §7 bullet phrased this as "`routes/maintainer.ts` imports `raceWithTimeout`" — that's not literally true; the route declares the injection point and `app.ts` wires it with the timed call.
- **Risk.** When PR-B moves Maintainer into a module, the module's `mount(ctx, deps)` function takes over what `app.ts` is doing today. The module needs `raceWithTimeout` to build the same wrapped `listSessions` closure, but the helper lives under another module's directory (`routes/sessions.ts` is Sessions-the-core-feature, not a shared util). Importing it cross-module violates the "low coupling" goal and creates an import-path landmine for the Sessions module's eventual modularization.
- **Disposition.** `promoted-to-CityContext`. Move `raceWithTimeout<T>(p: Promise<T>, ms: number): Promise<T>` into a shared util location accessible to both modules — concretely `backend/src/lib/raceWithTimeout.ts` (sibling of `lib/sanitise-error.ts`) — and have both `routes/sessions.ts` and the maintainer module import it from there. This is **NOT** literally a `CityContext` field; it's a host-provided utility. Calling it `promoted-to-CityContext` here flags it as "must be hoisted into a shared seam before PR-B" without prescribing the exact location. Update PR-A1's adapter scope or open a follow-up bead under PR-B1.
- **Status (PR-B1):** RESOLVED. Hoisted to `backend/src/lib/race-with-timeout.ts` (kebab-case to match `sanitise-error.ts` neighbour). `routes/sessions.ts` and the maintainer module descriptor (`maintainer.module.ts`) both import from there; the per-route timeout message text was generalised to "operation timed out after Xms" (no test was pinning the old "sessions route timed out" string). The old export from `routes/sessions.ts` is gone — no caller imports it from there anymore.
...
- **Disposition.** `promoted-to-CityContext`. The PRD's PR-A scope already includes *"Adapts MaintainerRefresher to `BackgroundWorker` shape so PR-B has a typed target"* (§7 PR-A row). Implementation needs: (a) `stop(): void` → `async stop(): Promise<void>` returning a resolved promise after `clearTimeout`/`clearInterval`; (b) drop the `running` getter from the public interface (keep as internal state if useful for tests). The `BackgroundWorker` interface lives in `shared/src/views.ts` per PRD §1. No PRD change needed — this is exactly what the PRD asks for.
