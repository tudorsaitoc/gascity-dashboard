...
- Keep a thin handwritten wrapper around the generated client so existing callers keep using methods like `listSessions()`, `listBeads()`, `getWorkflow()`, and `fetchTranscript()`.
...
- `GET /v0/city/{cityName}/workflow/{workflow_id}?scope_kind=...&scope_ref=...`
...
- Add one focused unit test proving generated path params are used correctly for `cityName`, `workflow_id`, and session `id`.
...
- Existing `workflows.test.ts` session stream test passes.
...
- `WorkflowRunDetail` should remain dashboard-owned because it is enriched display data.
- `WorkflowDiffResponse` remains dashboard-owned because it is produced by this backend, not supervisor.
...
- The supervisor schema may include broad `string` or weak `unknown` shapes for graph.v2 workflow details. If so, generation still helps with path/query/response status correctness, but the workflow detail enricher will still need runtime-tolerant parsing.
...
Do not combine this with workflow UI changes, browser e2e setup, or broad shared-type refactors.
