...
| ----------------------------------------------- | ------------: | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TDD                                             |            86 | 63 test files cover backend, frontend, shared, run detail, route validation, and fixtures. CI runs app and test typechecks plus backend/frontend tests. | Missing focused red tests for the audit-log path env parsing bug and the dolt-noms sampler source.                                                               |
...
| DRY                                             |            82 | Run detail logic is split into focused helpers, snapshot cache is reusable.                                                                             | Route catch/log/response handling is repeated, and several timer/refresh patterns are hand-coded per route.                                                      |
| Separation of Concerns                          |            87 | Routes, middleware, snapshot collectors, run projection, and shared types are separated.                                                                | Logging is scattered through route code, and snapshot service still owns placeholder product sources.                                                            |
| Single Responsibility Principle                 |            82 | Many backend run helpers have narrow responsibilities.                                                                                                  | `frontend/src/routes/Maintainer.tsx` and `backend/src/exec.ts` remain large multi-concern modules; `server.ts` wires many policies inline.                       |
...
| Low Coupling, High Cohesion                     |            83 | Runs, snapshot collectors, middleware, and API clients are cohesive.                                                                                    | `DashboardSources` forces consumers/tests to carry unused deferred sources; route handlers directly know logging details.                                        |
...
| YAGNI                                           |            76 | The main dashboard views are constrained to operator runs.                                                                                              | `aimux`, `github`, and `tokens` snapshot sources are explicitly not wired and unused by the visible run page.                                                    |
...
| No Placeholder Code                             |            65 | Run detail implementation is real and tested against graph.v2.                                                                                          | `sampleDoltNomsSize()` is a documented stub; snapshot deferred collectors are intentionally not wired.                                                           |
...
| Eliminate Race Conditions                       |            84 | `GcClient` and `SourceCache` use single-flight; run selection has tests for stale-cache refresh.                                                        | Config and logging state are global; sampler ring buffer failure paths are unobserved.                                                                           |
...
properties to mean several different things. For run run detail this made
...
still casts raw JSON to TypeScript types. The run detail projection now
...
that are outside the focused formula-run detail product. Those should be
handled in their own passes rather than mixed into this run refactor.
...
- Replaced run-detail nullable/optional app-owned fields with explicit
  semantic state:
  - `RunFormula`
  - `RunExecutionPath`
  - `RunSnapshotSequence`
  ...
  - `RunAttemptSummary`
  - `RunNodeScope`
- Made run display nodes identify their visible execution instance
  explicitly.
- Made run display edges carry an explicit kind, defaulting unknown
  dependency edges to `dependency`.
- Made diff errors carry a required error string instead of optional message
  data.
- Updated run fixtures, frontend tests, backend tests, and the browser
  run-detail harness to enforce the new contract.
- Logged unexpected degraded backend paths in builds, health, snapshot, SSE,
  and run diff handling while keeping expected absence states explicit.

## Reassessment After This Pass

| Rubric item                                     | Current score | What improved                                                                                           | Remaining gap before 100                                                                                      |
| ----------------------------------------------- | ------------: | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| TDD                                             |            94 | Added focused regression tests and updated run/browser fixtures to fail on stale contracts.             | Need equivalent edge-boundary validation tests for every supervisor response family, not only run detail.     |
| Consider First Principles                       |            90 | Run detail now models domain states directly instead of hiding absence.                                 | Snapshot deferred sources still need a first-principles product pass.                                         |
| Leverage Types                                  |            94 | `exactOptionalPropertyTypes`, required node/edge fields, and run discriminated unions close major gaps. | Supervisor fetches still cast raw JSON.                                                                       |
| DRY                                             |            86 | Logging adapter removes raw console duplication.                                                        | Route error response handling is still repeated.                                                              |
| Separation of Concerns                          |            90 | Run projection is more clearly a domain model over supervisor data.                                     | Snapshot composition still mixes active and deferred product sources.                                         |
| Single Responsibility Principle                 |            84 | Run detail helpers became more precise.                                                                 | `Maintainer.tsx`, `exec.ts`, and `server.ts` remain large.                                                    |
| Clear Abstractions & Contracts                  |            92 | Run run detail has an explicit UI contract instead of nullable holes.                                   | API error envelopes and supervisor validation need the same treatment.                                        |
| Low Coupling, High Cohesion                     |            88 | Run UI is a view over a richer backend projection.                                                      | Snapshot consumers still carry unused source concerns.                                                        |
...
| KISS                                            |            86 | Nullable guessing was removed from run UI logic.                                                        | The app still contains deferred source complexity.                                                            |
| YAGNI                                           |            80 | Run detail work stayed architectural, not feature expansion.                                            | Deferred sources are still present.                                                                           |
...
| Layered Architecture                            |            90 | Run detail has a stronger projection layer between supervisor and UI.                                   | Generated supervisor client/runtime validation is still missing.                                              |
| Use Non-Nullable Variables                      |            90 | Run-detail app-owned nullability was replaced by tagged states.                                         | Snapshot and maintainer domains still use null-heavy shapes.                                                  |
| Use Async Notifications                         |            84 | No regression; run/session SSE browser path is verified.                                                | Health and some ambient pages still poll.                                                                     |
...
| Keep Serialization/Deserialization At The Edges |            84 | Run projection rejects more malformed supervisor states.                                                | Raw supervisor JSON still enters through unchecked generic casts.                                             |
...
- Made every `GcClient` read pass through an endpoint-specific decoder before
  routes or run projection code can consume the value.
...
| Separation of Concerns                          |            92 | Fetching, decoding, route behavior, and run projection are more clearly separated.                                                                       | Snapshot composition still mixes active and deferred source concerns.                                                                  |
...
| Arrange Project Idiomatically                   |            92 | Tests and type gates remain idiomatic and green.                                                                                                         | CI should include the run-detail browser harness if it is not already required.                                                        |
...
- Migrated mail, sessions, beads, runs, snapshot, session stream, agents,
...
- Kept special behavior local where it is actually special: SSE proxy behavior,
  Health's degraded aggregate response, expected command exits, and run
...
| Rubric item                                     | Current score | What improved                                                                                                               | Remaining gap before 100                                                                                               |
| ----------------------------------------------- | ------------: | --------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| TDD                                             |            97 | Route-error behavior has focused red/green coverage, plus backend, frontend, and browser run-detail verification completed. | Remaining architecture work still needs the same red/green treatment, especially snapshot source states.               |
...
- Replaced fixture placeholder sources with explicit unavailable states for
  deferred sources.
- Updated snapshot composition, fixture loaders, run map rendering, and
...
run lane fields, and external supervisor shapes still have legitimate or
...
| Rubric item                                     | Current score | What improved                                                                                                                                       | Remaining gap before 100                                                                                                  |
| ----------------------------------------------- | ------------: | --------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| TDD                                             |            98 | Source-state semantics were changed with red/green cache tests, snapshot route tests, backend/frontend suites, and browser run-detail verification. | Remaining large-module and generated-client work still needs the same focused test scaffolding.                           |
...
| Use Non-Nullable Variables                      |            94 | The central snapshot source envelope no longer uses null for missing data, timestamps, or no-error states.                                          | Headline counts, maintainer triage data, run metadata, and external supervisor shapes still contain null-heavy contracts. |
| Use Async Notifications                         |            84 | No change.                                                                                                                                          | Health and ambient pages still poll.                                                                                      |
| Eliminate Race Conditions                       |            87 | No new async state was added.                                                                                                                       | Global worker/cache state remains only partially exercised.                                                               |
...
| Arrange Project Idiomatically                   |            93 | Shared contract, src/test typechecks, lint, backend/frontend tests, and browser harness all pass.                                                   | CI should include the run-detail browser harness if it is not already required.                                           |
...
| Separation of Concerns                          |            96 | Snapshot service no longer owns future/deferred product concerns.                                 | Maintainer and run detail modules still have some mixed presentation/data-prep responsibilities.                          |
...
| YAGNI                                           |            91 | Deferred snapshot sources are no longer part of the runtime shape.                                | Some historical comments and null-heavy maintainer/run fields still reflect future or past assumptions.                   |
...
| Use Non-Nullable Variables                      |            94 | Fewer source branches and no unavailable deferred source placeholders.                            | Headline counts, maintainer triage data, run metadata, and external supervisor shapes still contain null-heavy contracts. |
| Use Async Notifications                         |            84 | No change.                                                                                        | Health and ambient pages still poll.                                                                                      |
| Eliminate Race Conditions                       |            87 | No new async state was added.                                                                     | Global worker/cache state remains only partially exercised.                                                               |
...
| Arrange Project Idiomatically                   |            94 | Shared contract, src/test typechecks, lint, backend/frontend tests, and browser harness all pass. | CI should include the run-detail browser harness if it is not already required.                                           |
...
- Added focused `GcClient` tests proving generated city-scoped run and
...
| Separation of Concerns                          |            97 | JSON supervisor calls, SSE URL construction, route behavior, and runtime validation have clearer ownership.                | Maintainer and run detail modules still mix some presentation/data-prep responsibilities.                                 |
...
| YAGNI                                           |            92 | The pass generated only backend supervisor types and did not leak an SDK into the browser.                                 | Some historical comments and null-heavy maintainer/run fields still reflect future or past assumptions.                   |
...
| Use Non-Nullable Variables                      |            94 | Health no longer casts JSON directly and required dashboard health fields are validated before use.                        | Headline counts, maintainer triage data, run metadata, and external supervisor shapes still contain null-heavy contracts. |
...
| Arrange Project Idiomatically                   |            96 | OpenAPI tooling lives in npm scripts, generated files are committed, and lint ignores generated code only.                 | CI should include OpenAPI check and the run-detail browser harness if not already required.                               |
...
| Consider First Principles                       |            98 | Health now models degraded supervisor reachability as state, not absence.                                                    | Maintainer triage and run summary domains still need the same explicit modeling.                                          |
...
| YAGNI                                           |            92 | No new health feature was added; the existing degraded state became explicit.                                                | Some historical comments and null-heavy maintainer/run fields still reflect future or past assumptions.                   |
...
| Use Non-Nullable Variables                      |            95 | Removed `null` from the supervisor health contract.                                                                          | Headline counts, maintainer triage data, run metadata, and external supervisor shapes still contain null-heavy contracts. |
...
That let the Runs page render an unavailable run count as `0 active
runs`, hiding the distinction between "known zero" and "the source could
not answer."
...
- Updated fixture snapshot data and the Runs page synopsis so unavailable
  run counts are displayed as unavailable, not as zero.
- Added frontend regression coverage that prevents unavailable run counts
  from being flattened into "0 active runs."
...
- `node --import tsx --test backend/test/snapshot-route.test.ts` (red first,
  then green)
- `npm --workspace frontend test -- Runs.test.tsx`
- `npm run typecheck`
- `npm run lint`
- `node --import tsx --test backend/test/snapshot-route.test.ts backend/test/snapshot-cache.test.ts backend/test/snapshot-fixtures.test.ts backend/test/snapshot-failure-isolation.test.ts backend/test/snapshot-health-wiring.test.ts backend/test/snapshot-runs.test.ts`
...
| Consider First Principles                       |            99 | Headline now distinguishes known zero from unavailable data instead of treating absence as a number.                       | Run lane and maintainer domains still need the same explicit-state modeling.                                                         |
...
| Separation of Concerns                          |            97 | Snapshot service owns metric availability; Runs renders the shared state.                                                  | Maintainer view still mixes filtering, selection, dispatch, and rendering.                                                           |
...
| Low Coupling, High Cohesion                     |            96 | Runs no longer knows how to infer headline availability from nullable values.                                              | Large view modules still couple submodels and render logic.                                                                          |
| Scalability & Statelessness                     |            86 | No new app state was added.                                                                                                | Worker and ring-buffer assumptions remain single-process by design.                                                                  |
...
| YAGNI                                           |            93 | No new dashboard feature was added; the existing headline state became honest.                                             | Some historical comments and null-heavy maintainer/run fields still reflect future or past assumptions.                              |
...
| Use Non-Nullable Variables                      |            96 | Removed nullable headline counts and stopped treating unknown run counts as zero.                                          | City status internals, run lane metadata, maintainer triage data, and external supervisor shapes still contain null-heavy contracts. |
| Use Async Notifications                         |            85 | No change.                                                                                                                 | Health and ambient pages still poll.                                                                                                 |
| Eliminate Race Conditions                       |            89 | No new async state was added.                                                                                              | Global worker/cache state remains only partially exercised.                                                                          |
...
- `npm run typecheck`
- `npm run lint`
- `node --import tsx --test backend/test/snapshot-cityStatus.test.ts backend/test/snapshot-route.test.ts backend/test/snapshot-cache.test.ts backend/test/snapshot-fixtures.test.ts backend/test/snapshot-failure-isolation.test.ts backend/test/snapshot-health-wiring.test.ts backend/test/snapshot-runs.test.ts`

## Reassessment After City Status Metric Pass

| Rubric item                                     | Current score | What improved                                                                                    | Remaining gap before 100                                                                                      |
| ----------------------------------------------- | ------------: | ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- |
| TDD                                             |            99 | City summary used a failing collector test first, then shared type and snapshot-focused tests.   | Large-module extraction and run/maintainer domain-shape work still need focused red tests.                    |
| Consider First Principles                       |            99 | Empty sessions are now modeled as zeroes; only config-derived max sessions can be unavailable.   | Run lane and maintainer domains still need explicit-state modeling.                                           |
...
| YAGNI                                           |            93 | No new dashboard feature was added; existing city state became honest.                           | Some historical comments and null-heavy maintainer/run fields still reflect future or past assumptions.       |
...
| Use Non-Nullable Variables                      |            97 | Removed nullable city aggregate counts and nullable city max-session output.                     | Run lane metadata, maintainer triage data, and external supervisor shapes still contain null-heavy contracts. |
| Use Async Notifications                         |            85 | No change.                                                                                       | Health and ambient pages still poll.                                                                          |
| Eliminate Race Conditions                       |            89 | No new async state was added.                                                                    | Global worker/cache state remains only partially exercised.                                                   |
...
## Tenth Fix Pass: Run Census State

`RunSummary.census` still used `null` to mean "the health engine has not
...
- Changed the run summary builder test to require
  `{ status: 'unavailable', error: 'run health has not been derived' }`.
  It failed against the old `census: null` output.

Implemented:

- Added `RunCensusState`, a shared discriminated union:
  - `{ status: 'available', data: RunCensus }`
  - `{ status: 'unavailable', error: string }`
- Replaced `RunSummary.census: RunCensus | null` with
  `RunCensusState`.
- Updated the run summary builder and empty summary helper to emit an
...
- `node --import tsx --test backend/test/snapshot-runs.test.ts` (red
  first, then green)
- `node --import tsx --test backend/test/snapshot-runs.test.ts backend/test/snapshot-health-wiring.test.ts`
- `npm run typecheck`
- `npm run lint`
- `node --import tsx --test backend/test/snapshot-cityStatus.test.ts backend/test/snapshot-route.test.ts backend/test/snapshot-cache.test.ts backend/test/snapshot-fixtures.test.ts backend/test/snapshot-failure-isolation.test.ts backend/test/snapshot-health-wiring.test.ts backend/test/snapshot-runs.test.ts`
- `npm --workspace frontend test -- Runs.test.tsx`

## Reassessment After Run Census Pass

| Rubric item                                     | Current score | What improved                                                                                      | Remaining gap before 100                                                                                      |
| ----------------------------------------------- | ------------: | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| TDD                                             |            99 | Run census state used a failing builder test first, plus health wiring and frontend fixture tests. | Lane metadata and maintainer domain-shape work still need focused red tests.                                  |
| Consider First Principles                       |            99 | "Not derived yet" is now a first-class census state, not absence.                                  | Run lane and maintainer domains still need explicit-state modeling.                                           |
| Leverage Types                                  |            99 | Consumers can no longer treat run census as nullable.                                              | Generated runtime validation remains hand-written.                                                            |
| DRY                                             |            95 | Pre-engine census state is centralized in the run collector helper.                                | Large modules still duplicate rendering and command policy shape.                                             |
...
| Clear Abstractions & Contracts                  |            99 | Run summary now states whether census data is derived.                                             | Runtime supervisor validation is still local code.                                                            |
...
| YAGNI                                           |            93 | No new feature was added; existing census lifecycle became explicit.                               | Some historical comments and null-heavy maintainer/run fields still reflect future or past assumptions.       |
...
| No Placeholder Code                             |            94 | Fixture run data follows the explicit census contract.                                             | Remaining placeholder-like behavior is outside health/snapshot and needs separate audit.                      |
...
| Layered Architecture                            |            98 | Run collection and health enrichment communicate through explicit state.                           | Runtime validation should eventually be generated from the same schema.                                       |
| Use Non-Nullable Variables                      |            98 | Removed nullable run census.                                                                       | Run lane metadata, maintainer triage data, and external supervisor shapes still contain null-heavy contracts. |
| Use Async Notifications                         |            85 | No change.                                                                                         | Health and ambient pages still poll.                                                                          |
| Eliminate Race Conditions                       |            89 | No new async state was added.                                                                      | Global worker/cache state remains only partially exercised.                                                   |
...
| Keep Serialization/Deserialization At The Edges |            97 | Run summary serialization avoids nullable census output.                                           | Runtime decoder is still hand-written.                                                                        |
| Prefer Well-Known, High Quality OSS Libraries   |            97 | No new dependency was needed.                                                                      | Runtime validation generator choice remains open.                                                             |
| Treat Static Warnings And Info As Errors        |            97 | Typecheck and lint remain clean.                                                                   | Type-aware ESLint could be broader.                                                                           |
...
| Consider First Principles                       |            99 | Missing cache and corrupt cache are no longer conflated.                                     | Run lane and maintainer triage item fields still need explicit-state modeling.                                |
...
| YAGNI                                           |            93 | No recovery system was added; corruption is surfaced instead of hidden.                      | Null-heavy maintainer/run fields still reflect historical cache contracts.                                    |
...
| Use Non-Nullable Variables                      |            98 | `null` from `readCache` now has a single meaning.                                            | Run lane metadata, maintainer triage data, and external supervisor shapes still contain null-heavy contracts. |
| Use Async Notifications                         |            85 | No change.                                                                                   | Health and ambient pages still poll.                                                                          |
| Eliminate Race Conditions                       |            89 | No new async state was added.                                                                | Global worker/cache state remains only partially exercised.                                                   |
...
## Twelfth Fix Pass: Run Lane Scope State

Run lane navigation scope was split across three nullable/optional fields:
...
supervisor run detail endpoint.

Red test:

- Changed run summary tests to require a single `lane.scope` object. They
  failed because the builder still emitted the three loose fields.

Implemented:

- Added `RunLaneScope`, a shared discriminated union:
  - `{ status: 'available', kind, ref, rootStoreRef }`
  - `{ status: 'unavailable', error }`
- Replaced `RunLane.scopeKind`, `scopeRef`, and `rootStoreRef` with
  `RunLane.scope`.
- Updated the run lane builder to derive a complete scope from explicit
...
- `node --import tsx --test --test-name-pattern "run scope" backend/test/snapshot-runs.test.ts` (red first, then green)
- `npm --workspace frontend test -- LaneCard.test.tsx`
- `npm run typecheck`
- `npm run lint`
- `node --import tsx --test backend/test/snapshot-runs.test.ts backend/test/snapshot-health-wiring.test.ts backend/test/snapshot-runHealth.test.ts`
- `npm --workspace frontend test -- LaneCard.test.tsx Runs.test.tsx`

## Reassessment After Run Scope Pass

| Rubric item                                     | Current score | What improved                                                                         | Remaining gap before 100                                                                                                  |
| ----------------------------------------------- | ------------: | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| TDD                                             |            99 | Run scope used failing backend tests first, then frontend navigation tests.           | Remaining lane health/link fields and maintainer domain-shape work still need focused red tests.                          |
| Consider First Principles                       |            99 | Navigable scope is one complete state, not three independent maybe-fields.            | Other run lane fields still need explicit-state modeling.                                                                 |
...
| YAGNI                                           |            93 | No new run feature was added; existing navigation metadata became honest.             | Null-heavy maintainer/run fields still reflect historical cache contracts.                                                |
...
| Use Non-Nullable Variables                      |            98 | Removed three nullable/optional run lane scope fields.                                | Run lane health/link metadata, maintainer triage data, and external supervisor shapes still contain null-heavy contracts. |
| Use Async Notifications                         |            85 | No change.                                                                            | Health and ambient pages still poll.                                                                                      |
| Eliminate Race Conditions                       |            89 | No new async state was added.                                                         | Global worker/cache state remains only partially exercised.                                                               |
| Write for Maintainability                       |            97 | Future run-detail links cannot be built from partial scope state.                     | Large modules still need ownership-based extraction.                                                                      |
...
| Keep Serialization/Deserialization At The Edges |            98 | Run lane serialization now avoids nullable scope output.                              | Runtime decoder is still hand-written.                                                                                    |
| Prefer Well-Known, High Quality OSS Libraries   |            97 | No new dependency was needed.                                                         | Runtime validation generator choice remains open.                                                                         |
| Treat Static Warnings And Info As Errors        |            97 | Typecheck and lint remain clean.                                                      | Type-aware ESLint could be broader.                                                                                       |
| Use Centralized Semantic Constant Values        |            96 | Run scope availability is centralized in the shared contract.                         | Log component names and some polling/refresh constants are still stringly typed.                                          |

## Thirteenth Fix Pass: Run Lane Health State

Run lane health still used `health?: RunLaneHealth | null`, even
...
that lifecycle, which is the same ambiguity that previously hid broken run
detail behavior.

Red test:

- Changed the run summary builder test to require an explicit unavailable
  health state on newly built lanes. It failed because the builder emitted no
  health field.

Implemented:

- Added `RunLaneHealthState`, a shared discriminated union:
  - `{ status: 'available', data: RunLaneHealth }`
  - `{ status: 'unavailable', error: string }`
- Replaced `RunLane.health?: RunLaneHealth | null` with
  `RunLane.health: RunLaneHealthState`.
- Updated the run lane builder to emit
  `{ status: 'unavailable', error: 'run health has not been derived' }`.
- Updated snapshot health enrichment to replace that state with available
  health facts.
- Updated the run census builder to consume only available health.
...
- `node --import tsx --test --test-name-pattern "groups by metadata root|phaseConfidence|R2 fail-safe|health" backend/test/snapshot-runs.test.ts backend/test/snapshot-health-wiring.test.ts backend/test/snapshot-runHealth.test.ts` (red first, then green)
- `node --import tsx --test backend/test/snapshot-runs.test.ts backend/test/snapshot-health-wiring.test.ts backend/test/snapshot-runHealth.test.ts`
- `npm --workspace frontend test -- LaneCard.test.tsx Runs.test.tsx`
- `npm run typecheck`
- `npm run lint`

## Reassessment After Run Health-State Pass

| Rubric item                                     | Current score | What improved                                                                                     | Remaining gap before 100                                                                                      |
| ----------------------------------------------- | ------------: | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| TDD                                             |            99 | Run health lifecycle used a failing builder test first, then focused backend and frontend checks. | Full-suite and browser checks still need to be rerun after this latest shared type change.                    |
...
| DRY                                             |            96 | Pre-engine health state is centralized in the run lane builder.                                   | Large modules still duplicate rendering and command policy shape.                                             |
...
| Clear Abstractions & Contracts                  |            99 | Run lane health is now an explicit wire contract.                                                 | Supervisor validation is still a local implementation rather than generated.                                  |
...
| YAGNI                                           |            93 | No new product behavior was added; an existing lifecycle became explicit.                         | Null-heavy maintainer/run fields still reflect historical contracts.                                          |
...
| Layered Architecture                            |            98 | Run collection and health enrichment communicate through explicit state.                          | Runtime validation should eventually be generated from the same schema.                                       |
| Use Non-Nullable Variables                      |            98 | Removed optional/null lane health.                                                                | Run lane metadata, maintainer triage data, and external supervisor shapes still contain null-heavy contracts. |
| Use Async Notifications                         |            85 | No change.                                                                                        | Health and ambient pages still poll.                                                                          |
| Eliminate Race Conditions                       |            89 | No new async state was added.                                                                     | Global worker/cache state remains only partially exercised.                                                   |
...
| Keep Serialization/Deserialization At The Edges |            98 | Run lane serialization now avoids nullable health output.                                         | Runtime decoder is still hand-written.                                                                        |
| Prefer Well-Known, High Quality OSS Libraries   |            97 | No new dependency was needed.                                                                     | Runtime validation generator choice remains open.                                                             |
| Treat Static Warnings And Info As Errors        |            97 | Typecheck and lint remain clean.                                                                  | Type-aware ESLint could be broader.                                                                           |
| Use Centralized Semantic Constant Values        |            96 | Run health availability is centralized in the shared contract.                                    | Log component names and some polling/refresh constants are still stringly typed.                              |
...
The run-detail browser harness also drifted from the current snapshot wire
...
click the deterministic run lane. That is architecture debt in the test
...
error_kind=unknown` because the route responded before the audit append
  completed.
- `node scripts/snap-run-detail.mjs --test` failed waiting for the
  deterministic run lane because its mocked `/api/snapshot` used stale
  wire data.
...
- Updated `scripts/snap-run-detail.mjs` to emit the current snapshot
  contract:
  - `DashboardMetric` headline fields
  - only active sources (`city`, `resources`, `runs`)
  - current `SourceError` shape for available sources
  - current `RunLane.scope`
  - explicit pre-engine `RunLane.health`
  - explicit run census state
...
| Clear Abstractions & Contracts                  |            99 | The browser harness now validates current run navigation and detail contracts.                    | Supervisor validation is still a local implementation rather than generated.                                      |
...
| YAGNI                                           |            94 | No new product surface was added; stale harness data was removed.                                 | Null-heavy maintainer/run fields still reflect historical contracts.                                              |
...
| No Placeholder Code                             |            95 | The run-detail browser fixture follows the current real contract.                                 | Remaining placeholder-like behavior is outside this harness and needs separate audit.                             |
...
| Use Non-Nullable Variables                      |            98 | Harness data now uses explicit states instead of nullable headline and lane fields.               | Run lane metadata, maintainer triage data, and external supervisor shapes still contain null-heavy contracts.     |
| Use Async Notifications                         |            85 | No change.                                                                                        | Health and ambient pages still poll.                                                                              |
...
## Fifteenth Fix Pass: Run Lane External Reference State

Run lanes still carried external PR/issue metadata as two independent
...
- Changed the run summary tests to require `lane.external`:
...
- Added `RunLaneExternalReference`, a shared discriminated union:
  - `{ status: 'available', label, url }`
  - `{ status: 'label_only', label }`
  - `{ status: 'unavailable', error }`
- Replaced `RunLane.externalUrl` and `externalLabel` with
  `RunLane.external`.
- Updated the run lane builder to produce a single external-reference
...
- Updated fixtures, tests, and the run-detail browser harness.

Verification:

- `node --import tsx --test --test-name-pattern "external link" backend/test/snapshot-runs.test.ts` (red first, then green)
- `node --import tsx --test --test-name-pattern "external link|groups by metadata root|health" backend/test/snapshot-runs.test.ts backend/test/snapshot-health-wiring.test.ts backend/test/snapshot-runHealth.test.ts`
- `npm --workspace frontend test -- LaneCard.test.tsx Runs.test.tsx`
- `npm run typecheck`
- `npm run lint`
- `npm --workspace backend test`
...
| YAGNI                                           |            94 | No new product behavior was added; existing metadata became honest.                                           | Null-heavy maintainer/run fields still reflect historical contracts.                                                  |
...
| Use Non-Nullable Variables                      |            98 | Removed two nullable run lane external fields.                                                                | Run lane formula/progress, maintainer triage data, and external supervisor shapes still contain null-heavy contracts. |
| Use Async Notifications                         |            85 | No change.                                                                                                    | Health and ambient pages still poll.                                                                                  |
...
| Keep Serialization/Deserialization At The Edges |            98 | Run lane serialization avoids nullable external-link output.                                                  | Runtime decoder is still hand-written.                                                                                |
| Prefer Well-Known, High Quality OSS Libraries   |            97 | No new dependency was needed.                                                                                 | Runtime validation generator choice remains open.                                                                     |
...
## Sixteenth Fix Pass: Run Lane Formula State

Run lanes still carried the formula as `string | null`. That field is
...
- Changed run summary tests to require `lane.formula` as an explicit
  state:
...
- Added `RunLaneFormula`, a shared discriminated union:
  - `{ status: 'known', name }`
  - `{ status: 'unavailable', error }`
- Replaced `RunLane.formula: string | null` with
  `RunLane.formula: RunLaneFormula`.
...
- Updated snapshot fixtures, health tests, LaneCard tests, and the run
  detail browser harness fixture.

Verification:

- `node --import tsx --test --test-name-pattern "run formula|groups by metadata root" backend/test/snapshot-runs.test.ts` (red first, then green)
- `node --import tsx --test --test-name-pattern "run formula|groups by metadata root|phaseConfidence|health" backend/test/snapshot-runs.test.ts backend/test/snapshot-health-wiring.test.ts backend/test/snapshot-runHealth.test.ts`
- `npm --workspace frontend test -- LaneCard.test.tsx Runs.test.tsx`
- `npm run typecheck`
- `npm run lint`
- `npm --workspace backend test`
...
| Clear Abstractions & Contracts                  |            99 | Run lane formula is now an explicit wire contract.                                                            | Supervisor validation is still a local implementation rather than generated.                                         |
...
| YAGNI                                           |            94 | No new product behavior was added; existing metadata became explicit.                                         | Null-heavy maintainer/run fields still reflect historical contracts.                                                 |
...
| Use Non-Nullable Variables                      |            99 | Removed nullable run lane formula.                                                                            | Run lane progress fields, maintainer triage data, and external supervisor shapes still contain null-heavy contracts. |
| Use Async Notifications                         |            85 | No change.                                                                                                    | Health and ambient pages still poll.                                                                                 |
| Eliminate Race Conditions                       |            91 | No new async state was added.                                                                                 | Global worker/cache state remains only partially exercised.                                                          |
...
| Arrange Project Idiomatically                   |            97 | Shared type, backend builder, frontend component, fixtures, unit tests, and browser harness changed together. | CI should include OpenAPI check and browser harnesses if not already required.                                       |
| Keep Serialization/Deserialization At The Edges |            98 | Run lane serialization avoids nullable formula output.                                                        | Runtime decoder is still hand-written.                                                                               |
| Prefer Well-Known, High Quality OSS Libraries   |            97 | No new dependency was needed.                                                                                 | Runtime validation generator choice remains open.                                                                    |
| Treat Static Warnings And Info As Errors        |            97 | Full lint/typecheck remain clean after the shared wire change.                                                | Type-aware ESLint could be broader.                                                                                  |
...
After the headline, city status, run census, maintainer cache, run
scope, run health-state, audit-race, external-reference, formula-state,
progress/session-state, and execution-path-state passes, the broad suites and browser
run-detail harness were run to catch stale shared-wire assumptions:

- `npm run typecheck`: passed.
- `npm run lint`: passed.
- `npm --workspace backend test`: 495 tests passed.
- `npm --workspace frontend test`: 226 tests passed.
- `node scripts/snap-run-detail.mjs --test`: passed in light and dark.

## Seventeenth Fix Pass: Run Lane Progress And Session State

Run lanes still carried progress and health facts as nullable fields:
...
- Changed run summary tests to require:
...
- Changed run health tests to require:
...
- Added shared lane state types:
  ...
  - `RunLaneStuckNode`
  - `RunLaneSessionState`
- Replaced nullable progress fields on `RunLane` with
  `progress: RunLaneProgress`.
- Replaced nullable health session/stuck-node fields on `RunLaneHealth`
  with explicit state objects.
- Updated the run lane builder, health derivation engine, progress
  monotonicity marks, fixtures, LaneCard rendering, and the browser
  run-detail harness fixture.

Verification:

- `node --import tsx --test --test-name-pattern "explicit state|session facts|sessions unavailable|stuckNode" backend/test/snapshot-runs.test.ts backend/test/snapshot-runHealth.test.ts` (red first, then green)
- `node --import tsx --test --test-name-pattern "run formula|active run progress|groups by metadata root|phaseConfidence|health engine wiring|progress-monotonicity|city census" backend/test/snapshot-runs.test.ts backend/test/snapshot-runHealth.test.ts backend/test/snapshot-health-wiring.test.ts`
- `npm --workspace frontend test -- LaneCard.test.tsx Runs.test.tsx`
- `npm run typecheck`
- `npm run lint`
- `npm --workspace backend test`
...
| Leverage Types                                  |            99 | Run lane progress and health facts now force consumers through explicit states.                                              | Runtime validation remains hand-written rather than generated from the supervisor schema.                              |
...
| Separation of Concerns                          |            99 | Backend classifies run progress/session facts; frontend renders the resulting state.                                         | Maintainer view still mixes filtering, selection, dispatch, and rendering.                                             |
...
| Clear Abstractions & Contracts                  |            99 | Run progress and session availability are now explicit wire contracts.                                                       | Supervisor validation is still a local implementation rather than generated.                                           |
...
| Scalability & Statelessness                     |            86 | No new service state was added.                                                                                              | Worker and ring-buffer assumptions remain single-process by design.                                                    |
| Observability & Testability                     |            99 | Missing run progress/session facts are directly assertable without optional chaining.                                        | Frontend-side telemetry remains minimal.                                                                               |
...
| YAGNI                                           |            95 | No new product behavior was added; existing run facts became honest.                                                         | Null-heavy maintainer/run detail helpers still reflect historical contracts.                                           |
| Don't Swallow Errors                            |            99 | Missing run facts now carry reasons instead of disappearing into null.                                                       | Some command/resource cleanup paths still need the same audit.                                                         |
| No Placeholder Code                             |            96 | Browser and snapshot fixtures now use explicit progress/session state.                                                       | Remaining placeholder-like behavior is outside run lane progress and needs separate audit.                             |
...
| Use Non-Nullable Variables                      |            99 | Removed the nullable run lane progress and health session fields.                                                            | Maintainer triage data, run detail execution paths, and external supervisor shapes still contain null-heavy contracts. |
| Use Async Notifications                         |            85 | No change.                                                                                                                   | Health and ambient pages still poll.                                                                                   |
...
| Keep Serialization/Deserialization At The Edges |            98 | Run lane serialization avoids nullable progress/session output.                                                              | Runtime decoder is still hand-written.                                                                                 |
| Prefer Well-Known, High Quality OSS Libraries   |            97 | No new dependency was needed.                                                                                                | Runtime validation generator choice remains open.                                                                      |
...
## Eighteenth Fix Pass: Run Detail Execution Path State

The run detail route already serialized execution paths as
`RunExecutionPath`, but the resolver below it returned `string | null` and
...
- Changed `run-execution-path.test.ts` to require
  `resolveRunExecutionPath(...)` to return `RunExecutionPath`
  directly:
...
- Updated `resolveRunExecutionPath` to return `RunExecutionPath`.
- Removed the `runExecutionPathState` adapter from `formula-run.ts`; the
  formula-run projection now consumes the resolver's explicit state directly.

Verification:

- `node --import tsx --test backend/test/run-execution-path.test.ts` (red first, then green)
- `node --import tsx --test backend/test/run-execution-path.test.ts backend/test/run-enrich.test.ts backend/test/runs.test.ts`
- `npm run typecheck`
- `npm run lint`
- `npm --workspace backend test`
- `npm --workspace frontend test`
...
| Clear Abstractions & Contracts                  |            99 | `resolveRunExecutionPath` now advertises the same contract the route serves.                             | Supervisor validation is still a local implementation rather than generated.              |
...
| No Placeholder Code                             |            96 | Execution-path fixture and resolver tests now use the real explicit state.                               | Remaining placeholder-like behavior is outside run detail execution paths.                |
...
| Use Non-Nullable Variables                      |            99 | Removed `string                                                                                          | null` from run execution-path resolution.                                                 | Maintainer triage data and external supervisor shapes still contain null-heavy contracts. |
| Use Async Notifications                         |            85 | No change.                                                                                               | Health and ambient pages still poll.                                                      |
| Eliminate Race Conditions                       |            91 | No new async state was added.                                                                            | Global worker/cache state remains only partially exercised.                               |
...
| Keep Serialization/Deserialization At The Edges |            99 | Run detail path state is now classified before serialization instead of adapted near it.                 | Runtime decoder is still hand-written.                                                    |
| Prefer Well-Known, High Quality OSS Libraries   |            97 | No new dependency was needed.                                                                            | Runtime validation generator choice remains open.                                         |
...
| Use Centralized Semantic Constant Values        |            97 | Execution path absence reason is now centralized in the shared `RunExecutionPath` state.                 | Log component names and some polling/refresh constants are still stringly typed.          |
...
- `rg -n "openapi:gc-supervisor:check" .github/runs/ci.yml` returned no
  matches.
...
- `rg -n "openapi:gc-supervisor:check" .github/runs/ci.yml`
- `npm run openapi:gc-supervisor:check`
...
| DRY                                             |            98 | One generated-client check is shared by local and CI runs.                              | Large modules still duplicate rendering and command policy shape.                                       |
...
had no direct tests. The Runs page only mocked the hook contract. The hook
...
- Kept the existing `gcSupervisorDecoders` public surface so `GcClient`,
  routes, run projection, and UI contracts did not gain new coupling.
...
- `npm run typecheck`
- `node --import tsx --test backend/test/gc-client.test.ts backend/test/runs.test.ts`: 37 tests passed.
- `npm run lint`
- `npm --workspace backend test`: 505 tests passed.
...
rules in the run run detail session panel.
...
- Updated the run node session panel to keep rendering the last good
...
| Consider First Principles                       |         100 | The app now models actual domain states directly: source availability, headline metrics, health, city status, run census, execution paths, cache state, browser storage, and stream lifecycle.                                                                                                     | None.                    |
...
| Clear Abstractions & Contracts                  |         100 | Public contracts are now explicit and small: route errors, source states, metrics, health state, run run detail, generated supervisor types, Zod decoders, cache results, and session stream state.                                                                                                | None.                    |
...
| Use Centralized Semantic Constant Values        |         100 | Log components, source names, route error kinds, status unions, interval constants, regexes, and run/session state labels have centralized owners instead of scattered magic strings.                                                                                                              | None.                    |
...
- `node --import tsx --test backend/test/gc-client.test.ts backend/test/runs.test.ts`: 37 tests passed after the Zod runtime schema pass.
- `node scripts/snap-run-detail.mjs --test`: passed after the session stream state pass.
...
2. The branch is now architecturally much tighter than the starting point. The
   ...
   whether formula-run detail should become the primary organizing surface
...
- Frontend DRY/error/SRP: `frontend/src/components/ErrorBoundary.tsx`,
  ...
  and their tests.
- Static gates and docs: `eslint.config.mjs`, `.github/runs/ci.yml`,
...
| SRP                                   | Closed to 80+  | Agent detail display pieces, mail modal/message rendering, maintainer SSE/refresh/sling state, and common route utilities now sit in focused modules. `backend/src/routes/maintainer.ts` and run collectors remain large but have clearer helper boundaries and tests. |
...
| Async notifications                   | Closed to 80+  | Maintainer SSE has error reporting, run/event browser harnesses are isolated per route, and polling paths are either SSE-backed fallbacks or intentionally visibility-gated polling.                                                                                   |
...
| DRY                                             |            88 | The duplicated frontend helpers, backend route catch arms, JSON parsing, timeout helper, semantic constants, and shared types now have central owners.                                                                                   | Large run and maintainer modules still contain repeated policy-like shape in places where extraction would need a separate focused pass. |
| Separation of Concerns                          |            87 | App assembly, middleware, route error mapping, supervisor client access, formatting, error reporting, and maintainer actions are separated.                                                                                              | `backend/src/snapshot/collectors/runs.ts` remains broad because it owns a broad projection.                                              |
| Single Responsibility Principle                 |            84 | The largest frontend route responsibilities were split into display components and action hooks; backend maintainer storage/SSE/slung-state helpers are isolated.                                                                        | `backend/src/routes/maintainer.ts` and run collection still have multiple reasons to change, though no longer below the 80 bar.          |
...
| Low Coupling, High Cohesion                     |            87 | Shared package owns common wire contracts, route middleware owns cross-cutting behavior, and UI components consume focused hooks/helpers.                                                                                                | Run projection still couples several supervisor facts by necessity.                                                                      |
...
| Layered Architecture                            |            88 | Serialization/deserialization, route adapters, service construction, shared contracts, and UI rendering have clearer layers.                                                                                                             | Backend run projection is still a dense domain-adapter layer.                                                                            |
...
| Write for Maintainability                       |            89 | Common utilities, focused hooks/components, stricter CI, and architecture docs reduce future change cost.                                                                                                                                | The run collector should only be split further when a concrete change reason appears.                                                    |
...
- `node scripts/snap-run-detail.mjs --test`: passed in light and dark,
  including run detail load, diff states, node selection, historical
  iteration tabs, transcript peek, and session stream paths.
- `node scripts/snap.mjs --test`: passed for Agents, Beads, Runs, Mail,
  Activity, Health, and Triage in light and dark. Each recorded `/api/*`
  request returned 200.
- Visual inspection: `/tmp/cp-snaps/light-run-detail.png`,
...
3. `backend/src/snapshot/collectors/runs.ts` remains large. It is covered
   by focused run tests and has clearer helper boundaries, but further
   splitting should be driven by a concrete run projection change rather
...
`csells/run-detail-architecture-followup` worktree instead of assuming the
...
- The stale `exec.ts` shorthand saying the child received no inherited
  ...
  intentionally.
- The formula-run detail architecture spec no longer describes runtime
...
- The spec now names incremental event mutation and durable product metrics as
  ...
  requirements for the formula-run detail page.
...
- The actionable `arch-best-practices-02` items that overlap this branch's
  formula-run detail and supervisor-boundary scope are closed or captured as
...
- Remaining large-module and product-scope tradeoffs stay documented as
  ...
  longer block the formula-run detail architecture pass.
...
| Address stale security/architecture docs                            | `docs/SECURITY.md` already matches current ANSI-to-React rendering and exec env behavior; `backend/src/exec.ts` and `specs/architecture/formula-run-detail-type.md` were updated where stale wording remained.                                                                                 | Complete |
...
| Implement dashboard-owned formula-run detail architecture               | `FormulaRunDetail` remains the browser contract; `RunningFormulaRun` remains the backend aggregate; generated OpenAPI path/query/runtime validation, current git diff, session streamability, loop/retry instances, graph.v2-only handling, and event invalidation are implemented and tested. | Complete |
| Classify non-dashboard-owned formula-run target-state gaps              | Rig-store per-bead freshness, canonical Gas City/shared graph semantics, incremental projection mutation, durable product metrics, and upstream schema drift are documented as external constraints or future implementation moves, not hidden TODOs.                                      | Complete |
...
- `node scripts/snap-run-detail.mjs --test`: passed in light and dark, including run detail load, diff rendering, node selection, historical iteration tabs, transcript peek, session stream paths, partial snapshots, unsupported graph states, and diff edge states.
