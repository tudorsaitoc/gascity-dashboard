# PRD — Run Observability in the Gas City Dashboard
...
`run-observability-brainstorm.md`) → 3-lens diverge (IA, backend, product) → converge
(this doc) → premortem (Risks §).
**Grounding:** `run-observability-grounding.md` (sibling), `DESIGN.md`, `PRODUCT.md`,
...
surfaces (Runs lane strips, Maintainer triage queue, Activity log) and mentally OR them.
Her secondary question — **"what's happening with X?"**, answerable in two clicks — is
**unmet**: Runs rows are not clickable and there is no run-detail surface. The three
...
1. **Ship two NEW things, keep/repurpose the rest.** The two genuinely new surfaces are an
   **adaptive home (`/`)** that answers "is anything off?" and the **csells run-detail
   (`/runs/:id`)** that answers "what's happening with X?". Everything else is kept and
   demoted or enriched, not deleted.
2. **Attention has three conceptual depths but only two scan-surfaces.** The brainstorm's
   "L1 Needs-You" is **not a third route** — it renders as the _concern region of the home
   page_ (IA lens) and is _also_ a default view over the existing Maintainer surface
...
3. **One shared health derivation, computed once in the backend snapshot collector**, feeds
   ...
   stop disagreeing. MVP ships the _minimal_ version of this engine, not the full census.
...
L2 only serves the _less frequent_ investigation mode; you reach it only once you already know
...
| **Run-detail**                 | `/runs/:runId`                                  | **NEW (csells)**                 | "What's happening with X?" — vertical graph.v2 graph + Diff/Session split. Terminal drill-in.                                                                  |
| Runs index                     | `/runs`                                         | KEPT, demoted                    | Thin index; rows become deep links into run-detail.                                                                                                            |
...
- **Line 1 — phase-census (the <1s pattern-match target).** Headline scale, tabular figures,
  ...
  **no model call**. Its _shape_ changes when state changes (`nothing failing` → **`1 failing`**
...
- **Line 2 — template status sentence (the lean-in read).** Body scale, ≤70ch, prose
  assembled from structured facts: _"adopt-pr-271 has waited on a review verdict for 22 min;
  nothing is blocked, nothing is waiting on you."_

**One Mark Rule:** at most one maroon per viewport. It lands on the single most-severe **run-id
token** (rank-broken by oldest stall), and that token _is_ the deep link. Other concern clauses
use weight 600, not color. Greyscale-safe (carried by lede-position + weight + the word
"waited 22 min").

**Concern region:** when items need a decision, they materialize _beneath the sentence on the
same page_ via opacity (reserved space, never animate height; respects
...
- **Path A (alerted, primary):** the maroon run-id token in the sentence is
  `<a href="/runs/:id?node=:stuckNodeId">`. The engine already derived _which node_
...
- **Path C (browse):** `/runs` row → run-detail (no node preselected) → click a node.
...
stall (not just "this run is stuck"). `RunLane` gains a `stuckNodeId?` field.
...
- **`session.last_active` bumps on ANY tmux pane I/O (CONFIRMED).** `runtime/tmux/tmux.go:1898-1915`
  counts streaming output AND send-keys, so long _noisy_ compute correctly stays "active"; only a
  truly _silent_ step trips a false idle (R8 largely mitigated). It is the closest real activity
...
- **Resolution — the bead×session join + progress-monotonicity is the PRIMARY signal, not a
  ...
  available, classify a lane _stalled_ from: `max(bead.updated_at)` old **AND** the assigned
...
- **Engine location: backend snapshot read path** (resolved in P2a `gascity-dashboard-3ax`,
  corrected from the original "inside the runs load"). The derivation runs in the
  service composition layer _after_ both the runs source and a **shared sessions cache**
  settle — the runs loader fetches only beads and must not gain a 2nd `listSessions`
  ...
  P2a):** the server ships _facts_ — raw ISO timestamps, session liveness fields,
  `phaseConfidence`, and the one cross-cycle signal it alone can compute (`thrashingDetected`,
  the R1 progress-monotonicity result) — plus a _threshold-independent_ census (`byPhase`,
  confidence-scoped denominators). It does **not** ship a staleness-tier enum or a
  by-staleness-tier census: the staleness threshold crossing and _age display_ are computed in
...
- **csells reuse:** adopt `enrichFormulaRun` (`enrich.ts`) as the L2 backbone as-is — it
...
failed|blocked|skipped`), `attemptBadge` ("2/3"), iteration counts, session links, and the
  ...
  `shared/src/run-status.ts`** imported by both the list-driven L0 collector and the
  per-run L2 enricher, so "blocked / attempt-N" means one thing everywhere. Data _paths_ stay
  separate (city-wide list-beads for L0; per-run `getRun` for L2); _vocabulary_ unifies.

## 7. Sequencing

- **Phase 0 — Smooth the path for L2 (no branch rebase).** `csells/runs-formula-runs` is
  ...
  _their_ PR, on their schedule (bead `syl`, csells-owned). Everything above L2 deep-links _into_
...
- **Phase 4+ (evidence-gated).** Full health engine _only if a 2nd/3rd consumer appears_
  (YAGNI); Activity "Annotated Edition"; and the one phase-2 analytic bet (§9).

## 8. MVP scope

**In:** finished+merged csells L2; clickable Runs rows; favicon/tab ambient signal; one
...
this step"). It is the only analytic whose data dependency is _created by the MVP_ rather than
...
- **Three-surface-scan elimination** (route-nav logs: a "checking" session used to touch
  Agents+Health+Runs; after, it touches `/` and stops).
...
join, not deferred. _Leading indicator:_ first entry in the §10 Misses metric.

### R2 — The bead×session join is a cross-collector dependency the cache doesn't model (CRITICAL)

`collectors/runs.ts` fetches **only beads**; `session.last_active` lives in a _separate_
...
to the runs loader — read the existing cityStatus session data via the service layer;
...
_inline_ in `events.ts:82-119` (incl. a documented disconnect-while-backpressured connection-leak
...
imports. When _they_ next merge main into their branch (their schedule), it reconciles as a near
...
_Leading indicator:_ admin `rss_bytes` climbing across a day of drilling = the leak guard was lost
in their merge — re-check `sse-proxy.ts` survived intact.

### R4 — The shared `run-status.ts` extraction is unsound for status (HIGH)

L0 `phaseMapping.ts:46` derives phase from a substring sniff (`includes('blocked')`) → 8-value
_lane_ phase. L2 `enrich.ts presentationStatus` keys off `gc.outcome` → 9-value _node_ status.
...
The "· N more in triage" denominator covers the _triage_ slice, but inferred-confidence
_run_ runs (unmatched formula → generic fallback) fall in neither the census nor the
...
known)`. One inferred run changes the sentence _shape_. Never report calm over a population the
...
concern clause _is_ the calm signal. _Leading indicator:_ dwell-time rises on visits whose
sentence ended in a reassurance clause (she's re-checking it).

### R7 — §9 deviation bet starves the MVP (HIGH — most likely _design/scope_ death)

"Near-zero-marginal-cost" is the lie: per-step "typical" needs a persistence layer (the
dashboard has none), a baseline backfill, outlier handling, and a _semantic_ "what is typical"
...
first on _absolute_ attempt-velocity (no baseline needed), adding the K×-typical refinement only
in Phase 4.
...
_truly silent_ step trips a false idle, a much narrower window. The acute version of this risk is
...
Tier is computed once and frozen in the cached snapshot for up to `RunS_CACHE_TTL_MS=60s`
(`runs.ts:40`); a stall crossing at second 5 stays green until second 60 while the client
ticks the age label past the threshold — the worst trust signal (the number that should alarm,
shown calm). Worse, `Runs.tsx:63` skips bypass-refresh when source is `stale`, pinning the
...
### R10 — Two-surface overlap (`/` vs `/runs`) never resolved → merges back into sprawl (MEDIUM)

§3 admits route count nets flat and never defines what `/runs` shows that `/` withholds, so
`/` accretes healthy runs "while we're here" and `/runs` becomes vestigial — re-creating
"everything competes for attention."
**[SPEC CHANGE to §3] Withholding contract:** `/` shows ONLY the census line + items that need a
decision or are failing/stalled; it **never lists a healthy in-flight run**. `/runs` is the
only place a calm in-flight run is enumerated. Test: on a fully-calm city, `/` shows the census
line and nothing else; `/runs` shows N healthy rows. A calm run on `/` = rejected PR.

### R11 — Drift to Datadog: census line → labeled KPI count-grid (MEDIUM)

The csells page header _already_ uses a four-column labeled count strip (`STATUS / DONE / PENDING
...
`/runs/:id`; assert **≤1 maroon-class element per viewport** in the DOM. Rule: Line 1 is a
...
violates **The Flat Page Rule** (scope boxes _are_ containers). The SVG's color restraint masks
...
one place containers are structural (a run _is_ a dependency graph; space+type can't encode
...
`/needs-you` → a nav entry → four scan-surfaces, _more_ than the original three.
...
filter. Review test: _does this add a router entry? Then it's a place, and it's rejected._

### The single highest-leverage guardrail

Make `scripts/snap.mjs` + Greyscale Test a **CI gate on `/` and `/runs/:id`** with a "≤1
...
claims about _absence_ its data can't guarantee, in a voice calibrated to be trusted absolutely.**
The unifying fix: report presence with confidence; report the _boundary of your own knowledge_
