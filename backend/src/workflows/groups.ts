...
import type { WorkflowNodeGroup } from './execution-instances.js';
import {
  badgeLabelFor,
  constructKindFor,
  displayTitleFor,
...
function shouldPromoteGroupShape(
  current: WorkflowNodeGroup['constructKind'],
  candidate: WorkflowNodeGroup['constructKind'],
): boolean {
...
function resolveSemanticIds(
  beads: readonly GcWorkflowBead[],
  rootBeadId: string,
  physicalLogicalTargets: ReadonlySet<string>,
): Map<GcWorkflowBead, string> {
  const baseIds = new Map<GcWorkflowBead, string>();
  ...
  const resolved = new Map<GcWorkflowBead, string>();
  for (const bead of beads) {
...
function buildBadgeTargetAliases(
  beads: readonly GcWorkflowBead[],
  rootBeadId: string,
  semanticIds: Map<GcWorkflowBead, string>,
  physicalLogicalTargets: ReadonlySet<string>,
): Map<string, string> {
...
function resolveBadgeTarget(
  ...
  if (constructKindFor(bead, rootBeadId) === 'workflow-finalize') return rootBeadId;
...
function hiddenBadgeFullTargetFor(bead: GcWorkflowBead): string | undefined {
