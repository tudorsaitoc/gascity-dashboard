...
import {
  ...
  nonEmpty,
  normalizedStepRef,
} from '../src/runs/bead-fields.js';

describe('run bead field readers', () => {
  ...
  test('falls back to supervisor fields and step ref segments', () => {
    assert.equal(attemptFor(runBead({ attempt: 4 })), 4);
    assert.equal(iterationFor(runBead({
      step_ref: 'mol.review.iteration.2.apply',
    })), 2);
    assert.equal(attemptFor(runBead({
