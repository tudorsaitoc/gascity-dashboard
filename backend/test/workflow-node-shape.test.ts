...
import {
  ...
  semanticNodeIdFor,
} from '../src/workflows/node-shape.js';

describe('workflow node shape projection', () => {
  test('derives semantic ids by precedence without exposing private names', () => {
    assert.equal(semanticNodeIdFor(workflowBead({ id: 'root' }), 'root'), 'root');
    assert.equal(
      semanticNodeIdFor(workflowBead({
  ...
  test('maps supervisor kinds to public construct kinds', () => {
    assert.equal(constructKindFor(workflowBead({ id: 'root' }), 'root'), 'workflow-root');
    ...
    assert.equal(kindFor('workflow-finalize'), 'workflow-finalize');
  ...
  test('external kind labels also suppress private names', () => {
    const bead = workflowBead({ metadata: { 'gc.kind': 'ralph' } });

    assert.equal(externalKindFor(bead, 'check-loop'), 'check-loop');
    assert.equal(externalKindFor(workflowBead({ kind: 'task' }), 'step'), 'task');
    assert.equal(externalKindFor(workflowBead({ kind: ' ' }), 'step'), 'step');
  });

  test('computes hidden control badge targets and labels', () => {
    assert.equal(
      hiddenBadgeTargetFor(workflowBead({
        metadata: {
    ...
    assert.equal(badgeLabelFor('workflow-finalize'), 'finalize');
  ...
  test('classifies hidden/control constructs and visible constructs', () => {
    ...
    assert.equal(isHiddenConstruct('workflow-finalize'), true);
  ...
  test('derives loop control ids from iteration step refs', () => {
    assert.equal(
      loopControlNodeIdFor(workflowBead({
    ...
    assert.equal(loopControlNodeIdFor(workflowBead({ step_ref: 'mol.review' })), undefined);
  });

  test('uses title when present and falls back to readable ids', () => {
    assert.equal(displayTitleFor(workflowBead({ title: ' Review pipeline ' }), 'fallback'), 'Review pipeline');
    assert.equal(displayTitleFor(workflowBead({ title: ' ' }), 'review-pipeline'), 'review pipeline');
  });

  test('suppresses private check-loop names in display titles', () => {
    assert.equal(
      displayTitleFor(workflowBead({ title: 'Review ralph pass' }), 'fallback'),
      'Review check loop pass',
    );
    assert.equal(
      displayTitleFor(workflowBead({ title: 'mol_ralph_review' }), 'fallback'),
      'mol check loop review',
    );
    assert.equal(
      displayTitleFor(workflowBead({ title: 'cryoralphic analysis' }), 'fallback'),
      'cryoralphic analysis',
    );
  });
});

function kindFor(kind: string): WorkflowConstructKind {
  return constructKindFor(workflowBead({ kind, metadata: { 'gc.kind': kind } }), 'root');
}
